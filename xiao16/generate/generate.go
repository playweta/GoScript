...
var rand24 = "QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm"
var number10 = "1234567890"
var number10sp = strings.Split(number10, "")
var rand24sp = strings.Split(rand24, "")
var rand14Len int = len(rand24sp)
var number10spLen int = len(number10sp)
...
func RandomZNName() string {
	...
	sprintf := fmt.Sprintf("%s%s", fmt.Sprint(lastName[rand.Intn(lastNameLen-1)]), first)

	return sprintf //, dict.Sentence(sprintf).None()
}

func RandomENName() string {
	lock.Lock()
	count = count + 1
	lock.Unlock()
	rand.Seed(time.Now().UnixNano() + int64(count)) //设置随机数种子

	name := fmt.Sprintf("%v %v", GnMen[rand.Intn(len(GnMen))], NAMES[rand.Intn(len(NAMES))])
...
func RandNumber(len int) string {
	...
	var first string
	for i := 0; i <= len; i++ {
		first += fmt.Sprint(number10sp[rand.Intn(number10spLen-1)])
	}
...
func Randletter(len int) string {
	...
	var first string
	for i := 0; i <= len; i++ {
		first += fmt.Sprint(rand24sp[rand.Intn(rand14Len-1)])
	}
